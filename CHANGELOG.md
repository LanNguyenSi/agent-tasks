...
**docs/okf quoted-literal drift guard (T-007, tracker 3feb590f).** Batch-43 (PR #509) re-stamped six bundle docs under okf-kit's `sources-fresh` check, which only compares timestamps against last-commit time and so missed two now-fixed claims a reviewer's source read caught: `SERVER_VERSION` quoted at an old value, and a teaching-error `allowedNext` array quoted with the wrong verbs. A new backend test (`backend/tests/unit/docs-okf-literal-guard.test.ts`) now checks every VALUE literal (semver, quoted string, JSON-shaped bracket/brace, `key: value`) that sits within 30 words of a `path:N`/`path:N-M` line citation, in the same markdown block, against the cited source lines; bare identifiers are never checked. Measured against the current bundle: 13 value literals in scope (12 across architecture.md/deploy.md/mcp-server.md/workflow-gates.md/log.md's live paragraphs, plus `log.md`'s own history-only literals counted separately and never asserted), 244 bare identifiers correctly left unchecked, 2 allowlisted (`deploy.md`'s `test:coverage` and `needs: mcp-server`, both true statements paired by word-proximity with a citation describing a different clause of the same five-job enumeration), 0 unallowlisted findings after fixing two citation drifts found live by the guard while building it: `governance-merge.md`'s `github.ts:352` off-by-one (the `task.status === "review"` check is actually on line 351) and `mcp-server.md`'s `mcp-server/src/server.ts:9` widened to `:8-9` (it was citing only `SERVER_VERSION`, one line below the `SERVER_NAME` literal the same sentence also quotes). Decision record and the two rejected options (an okf-kit rule proposal; process-only re-stamp discipline) in `docs/okf/index.md`'s new Maintenance section. See `docs/okf/governance-merge.md`, `docs/okf/mcp-server.md`.
