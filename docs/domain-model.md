...
| `TaskDependency` | Many-to-many self-relation on `Task` modelling `blocks` / `blockedBy`. Cycle-checked. `task_pickup` skips a task until every `blockedBy` parent reaches a resolved status (`done` or `abandoned`). |
...
`Task.status` is a free `String` because custom workflows can define their own states. The product status vocabulary includes six canonical values: `backlog` (agent-created tasks awaiting human review, handled outside the workflow engine), `open`, `in_progress`, `review`, `done` (the four states that compose the default workflow engine), and `abandoned` (terminal, only reachable via human backlog discard or admin reset). Agent creates route to `backlog` automatically via dedicated claim routing; human creates default to `open`. The workflow engine transitions only between the four engine states; `backlog` and `abandoned` are routed via dedicated operator/admin surfaces, not engine transitions.
...
`GroundingAttempt` stores the exact canonical context bytes, server-derived workflow target, nonce, owner, lifetime and active/superseded/consumed state. A direct attempt also stores a strict `{version: 1, endpoint, target}` route descriptor, which is reauthorized on ingest and cannot cross the v2 attempt lane. The producer session tuple is nominated only inside a successful receipt-ingest transaction. `GroundingReceipt` stores one immutable accepted original wire representation and its documentary `agent_asserted` evidence per attempt. Composite foreign keys preserve task identity across these records. `GroundingFinalization` holds receipt-backed consumption/reservation detail and preserves the same-task receipt foreign key. `GroundingCohort` pins explicit server-owned EXTERNAL_V1/LEGACY_LOCAL/OFF protection/provenance and optional legacy reference, plus one active reservation. `GroundingOperation` stores immutable task/key/actor/request identity and selected decision, with RESERVED, DISPATCHED, COMPLETED or CANCELLED state and a durable result. Receipt-free decisions need no Finalization row.

These additive tables leave `Project.requireGroundingForDebug` defaulting to false. Attempt issuance and receipt ingest do not change task status, claims or merge state. A server-owned optional creation-policy entry can atomically create a new task with its cohort and binding; absent selection remains the historical path. Enrolled history prevents task or project deletion with `grounding_history_retained`; that response retains data and does not implement archival. See [the grounding receipt contract](grounding-receipt-contract.md) for exact context bytes, API inputs, uniqueness and retry behavior.
