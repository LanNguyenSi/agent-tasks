...
**Backlog filtering in pickup**: `POST /tasks/pickup` does not return backlog tasks at any point in its resolution order. A task in `backlog` status is invisible to both the signals branch (backlog tasks emit no `task_available` signal; see `task-lifecycle.md`) and the open-work branch. Backlog tasks are intended for human review before agent pickup; see `task-lifecycle.md` for the human promote/discard surface.
