...
configured app's fresh remote operations on an unenrolled task fail closed
with `409 grounding_enrollment_required` rather than falling back to the
unprovisioned path (see the receipts paragraph above). Full API and
