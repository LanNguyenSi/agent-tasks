  ...
  const tools: ToolDefinition[] = [
    ...
    def({
      ...
      description:
        ...
        "The response carries `nextCursor` (a task id, or null once the last page is reached); pass it back as `cursor` to page forward; combined with `sort` + `id` as a tiebreaker, page order is stable even when many tasks share the same createdAt timestamp. It also carries `count` (rows in this page) and `truncated` (true exactly when more rows exist after this page, i.e. nextCursor is non-null); the backend proves this exactly via a take-limit-plus-one probe, so a page whose length happens to equal `limit` is never misread as complete when `truncated` says otherwise. " +
        "Rows are a SUMMARY by default: an ALLOWLIST of id, title, status, priority, labels, externalRef, createdAt, claims, blockedBy, prUrl (plus totalLabels / totalBlockedBy when either list is clamped); every other backend field (description, templateData, blocks, branchName, prNumber, result, dueAt, claimedAt, metadata, deliverableRepo, workflowId, updatedAt) is left off each row, not merely description/templateData, so `limit` alone is no longer the only lever against the token cap; a listing of tasks with long descriptions stays small without lowering `limit`. `claims` on this route can only carry a work claim (`claims.work`): this route's rows never carry a review claim. Pass include:[\"description\"] or include:[\"templateData\"] to add just that field back to every row, or include:[\"task\"] for the full, pre-contract rows.",
      inputShape: {
        project: z
          .string()
          .trim()
