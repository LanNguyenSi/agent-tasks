...
The converted v2 write verbs (task_create, task_respec, task_finish, task_submit_pr, task_note, task_merge, task_abandon, tasks_comment) return small receipts by default and accept include:["task"]; task_pickup returns the full spec and task_start a receipt plus a small slice; every other tool returns the raw backend body and ignores include. Call workflow_primer (no arguments) for full lifecycle detail, include semantics, and today's error behavior; call projects_get_effective_gates before task_create or task_submit_pr to see this project's actual gates.`;
