...
The converted v2 write verbs (task_create, task_respec, task_finish, task_submit_pr, task_note, task_merge, task_abandon, task_creator_abandon, tasks_comment) return small receipts by default and accept include:["task"]; task_pickup returns the full spec and task_start a receipt plus a small slice; tasks_get returns a summary by default and accepts include:["description", "comments", "artifacts", "task"]; project_tasks returns summary rows by default and accepts include:["description", "templateData", "task"]; signals_poll caps and cursors the backend response locally even though it takes no include parameter; every other tool returns the raw backend body and ignores include. Call workflow_primer (no arguments) for full lifecycle detail, include semantics, and today's error behavior; call projects_get_effective_gates before task_create or task_submit_pr to see this project's actual gates.`;
...
- task_create, task_respec, task_finish, task_submit_pr, task_merge, task_abandon, task_creator_abandon, task_note, tasks_comment: a small receipt only, shaped { ok, task: { id, status? }, confidence?, deviations?, next? }. confidence is set only by task_create and task_respec (the score just computed for that call); every other verb in this list omits it. Fields you sent are never echoed back; deviations appear only when something needs your attention.
...
## Errors
A call that cannot proceed returns an MCP tool error (isError: true) whose text is the JSON teaching-error shape \`{ ok: false, error: { code, message, recipe, allowedNext, detail? } }\`, printed the same way every other tool result is (two-space-indented JSON), never a bare status or raw backend string. \`code\` identifies the failure, \`recipe\` names the concrete corrective call, and \`allowedNext\` lists only verb names you can call immediately (a strict, machine-checkable counterpart to a receipt's free-form \`next[]\`); \`detail\` carries structured extras for the codes that have them. Known traps and their recipe: \`not_claimed\` (403, calling task_finish, task_submit_pr, or task_abandon without holding the claim, call task_start first); \`already_claimed\` (409, a claim wall on task_pickup or task_start: finish or abandon your in-progress claim; a work claim held on an in-review task needs the review cycle completed, or wait, not abandon; a review claim can be abandoned); \`precondition_failed\` (422, an unmet branchPresent/prPresent/ciGreen/prMerged gate, \`detail.failed\` lists each failing rule individually, call task_submit_pr first when a branch or PR gate is the one failing); \`cross_repo_pr_rejected\` (400, task_submit_pr's prUrl points outside the project's own repo); \`pr_author_mismatch\` (403, that PR was not authored by the delegation user); \`force_admin_only\` (403, the admin-only force-transition escape hatch used as a non-admin, no self-service fix exists); \`respec_conflict\` (409, task_respec on a task that is claimed or not open); \`creator_abandon_conflict\` (409, task_creator_abandon on a task that is not open+unclaimed, or a lost CAS race, call tasks_get to see its current state); \`low_confidence\` (422, task_start's pre-claim confidence gate rejected the claim, detail carries score/threshold/missing[]/totalMissing, call task_respec to raise the score); \`result_not_plain_string\` (task_finish's own result rejected before any request is sent, because it looks XML- or JSON-wrapped instead of plain prose); \`project_addressing_conflict\` (task_create's project/projectId/projectSlug addressing rejected client-side before any request is sent, because exactly one of the three is required, resubmit with exactly one, or ask the operator which project to use if you do not yet know it); \`unknown_project_slug\` (task_create's or project_tasks's project/projectSlug value did not resolve to any project on a fresh lookup, ask the operator for the correct slug or id); \`backlog_routing_enforced\` (400, task_create rejected an explicit non-backlog status from an agent caller, omit status or pass status: "backlog"); \`backlog_not_promoted\` (403, task_start or task_pickup on a backlog task not yet promoted, call task_respec to refine it while it waits or task_creator_abandon to withdraw it). Any other backend error still degrades to the same shape instead of leaking raw text: a code derived from its HTTP status, the backend's own message passed through, a recipe pointing at workflow_primer, and, when the backend sent a structured details payload, a clamped copy of it in detail.

## Backlog routing (v1)
Every agent-created task lands in \`backlog\`, not \`open\` (task_create routes it there unconditionally; an explicit non-backlog status from an agent caller is rejected, not silently downgraded). task_pickup and task_start cannot see or start a backlog task (backlog_not_promoted, above); only an operator can promote it to open, or discard it. The creator can withdraw it any time via task_creator_abandon. task_respec works on a backlog task for any caller, same as on an open one, so it stays a draft space to refine before promotion; once promoted to open, task_respec reverts to creator-only.
